...
function sendUsage(message) {
  message.reply(
    `usage : \`${prefix}stage @participant1 @participant2 [option : tour2]\`\n For any questions ask Steyo`
  );
}
...
async function chooseStage(user1, user2, args, message) {
  ...
  while (levelsRemaining > 1) {
    ...
    if (levelsRemaining == 9) image1 = { files: [allStages] };
    else {
      await drawImages(ctx, images, canvas, levelsRemaining, levels);
      image1 = new Discord.MessageAttachment(
        canvas.toBuffer(),
    ...
    await message.channel
      .send(GetMessageToSend(userWanted, levelsNbToBan), image1)
      .then(async () => {
        let userMessage = await message.channel
          .awaitMessages(
            (response) => checkAnswers(response, levels, userWanted),
          ...
          .then((collected) => {
            ...
            if (reponses.length > levelsNbToBan[0])
              reponses.splice(
                levelsNbToBan[0],
                reponses.length - levelsNbToBan[0]
              );
  ...
  if (levelsRemaining === 1) {
    message.channel
      .send(`${user1} ${user2} Que le match commence !`, {
